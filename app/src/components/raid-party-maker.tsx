...
export default function RaidPartyMakerV3({ testMode = false }: { testMode?: boolean }) {
    ...
    const [algoCards, setAlgoCards] = useState<AlgoCard[]>([
        ...
        { id: 'safety_optimization', label: '안전 최적화', desc: '검성 탱커 시 호법성을 배치하여 생존력을 보강합니다.', active: true, status: 'PRIORITY' },
    ...
    const handleAutoMatch = () => {
        ...
        // 1. Scarcity Priority (Smart Sort by Availability Count)
        const matchScarcityPriority = (candidates: Member[]) => {
            // Sort candidates by number of available slots (Ascending)
            // Fewer slots = Higher Priority
            return [...candidates].sort((a, b) => {
                const countA = Object.values(a.availability || {}).flat().length;
                const countB = Object.values(b.availability || {}).flat().length;
                return countA - countB;
            });
            // Note: This function doesn't assign, it just reorders the candidate list for subsequent steps.
            // But the current architecture expects 'used' IDs to be returned if assigned.
            // Wait, 'Scarcity Priority' in V3 context (as a card) should probably be a "Sort Modifier" rather than an "Assigner".
            // However, existing functions (matchFixedParties) return 'used' IDs.
            // If we want Scarcity to affect *who* gets picked by *later* cards, we should just sort the 'candidates' array in place or return sorted.
            // But 'runAlgo' expects 'used' list.
            // Actually, if this card is active, we should probably just *sort* the working candidates for the *current* force loop.
            // But 'candidates' is local to the loop. 
            // Let's implement it as: "Refine Candidates List" (Mutate priority).
            // Standard AlgoCard pattern: It tries to fill spots.
            // But 'Scarcity Priority' is abstract. 
            // Let's make it: "Fill empty spots with most scarce people NOW".
            // Use Snake Draft logic but with Scarcity Sort.
        };

        // Revised Scarcity Match: Fill generic spots using Scarcity Sort
        const matchScarcityFill = (partiesToMatch: Party[], candidates: Member[]) => {
            const used: string[] = [];
            const sorted = [...candidates].sort((a, b) => {
                const countA = Object.values(a.availability || {}).flat().length;
                const countB = Object.values(b.availability || {}).flat().length;
                if (countA !== countB) return countA - countB; // Less slots = First
                return b.power - a.power; // Tie-break with Power
            });

            // 1-2-2-1 Snake Fill for fairness, or just fill holes?
            // Let's just fill linear for now to ensure scarce people get in.
        ...
        // 3. Resurrection Anchor (Essential)
        ...
        // 4. Safety Optimization (Priority)
        const matchSafetyOptimization = (partiesToMatch: Party[], candidates: Member[]) => {
            ...
            partiesToMatch.forEach(p => {
                ...
                // If Glad Tank AND No Templar AND No Chanter -> Add Chanter
        ...
        // 5. Combat Logic (Priority)
        const matchCombatLogic = (partiesToMatch: Party[], candidates: Member[]) => {
            const used: string[] = [];
            // Simple logic: If too many melee, add range. If too many range, add melee.
            // Melee: Glad, Templar, Sin, Chanter. Range: Sorc, Spirit, Ranger, Cleric(kinda)
            // Let's simplify: Aim for mix.
            ...
            partiesToMatch.forEach(p => {
                ...
                const ranges = p.members.filter(m => ['마도성', '정령성', '궁성'].includes(m.class)).length; // Exclude Cleric from count

                let targetType = '';
                if (melees > ranges + 1) targetType = 'RANGE';
                else if (ranges > melees + 1) targetType = 'MELEE';
                else return; // Balanced enough
                ...
                if (pick) {
                    p.members.push(pick);
                    used.push(pick.id);
                    // remove from local candidates to avoid double pick in loop? 
                    // (Actually candidates passed to this Func are static, need to track used)
        ...
        // 6. Attendance Volume (Priority)
        const matchAttendanceVolume = (partiesToMatch: Party[], candidates: Member[]) => {
            // Just fill empty spots effectively
            return matchPowerBalance(partiesToMatch, candidates); // Reuse Snake Draft for efficient filling
        };


        const handleAutoMatch = () => {
            if (pool.length === 0) return alert('대기 멤버가 없습니다.');
            const targetScope = matchOptions.targetScope;
            const timeScope = matchOptions.timeScope;
