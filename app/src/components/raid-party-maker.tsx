...
export default function RaidPartyMakerV3({ testMode = false }: { testMode?: boolean }) {
    ...
    const [algoCards, setAlgoCards] = useState<AlgoCard[]>([
        ...
        { id: 'scarcity_priority', label: '희소 직업/시간 우선', desc: '시간이 부족한 인원이나 귀족 직업군을 우선 배정하여 매칭 실패를 최소화합니다.', status: 'PRIORITY' },
        { id: 'safety_opt', label: '호법성 시너지', desc: '검성 탱커 파티나 탱커가 없는 파티에 호법성을 배치하여 생존력을 유지합니다.', status: 'PRIORITY' },
        { id: 'combat_logic', label: '전투 로직 (직업 밸런스)', desc: '근거리/원거리 클래스 비율을 균형 있게 맞춥니다.', status: 'PRIORITY' },
        { id: 'power_balance', label: '파워 밸런스', desc: '전투력 고점과 저점을 교차하여 평준화합니다.', status: 'PRIORITY' },
        { id: 'attendance_volume', label: '파티 구성 극대화', desc: '설정된 규칙을 준수하며 가능한 많은 파티가 완성되도록 인원을 최적 분배합니다.', status: 'PRIORITY' },
    ...
    useEffect(() => {
        if (testMode) {
            ...
            // Filter: Only include members who have applied (have availability)
            const applicants = mockMembers.filter(m => {
                if (!m.availability) return false;
                return Object.values(m.availability).some(slots => slots && slots.length > 0);
            });
            setPool(applicants);
        ...
        const loadMembers = async () => {
            // 1. Load Repo Members (Guild Roster)
            // Use onValue with onlyOnce for one-time fetch if 'get' is not imported, or just use existing pattern
            // To ensure we get both, we'll use Promises wrapper around onValue if needed, or just nested callbacks.
            // Let's use the clean Promise approach with get() if available.

            // Since I can't see imports, I will assume 'get' is available or I will add it to imports in a separate step if needed. 
            // Actually, I should use `onValue` as it's already used.

            const membersRef = ref(db, 'members');
            onValue(membersRef, (membersSnap) => {
                const membersData = membersSnap.val() || {};
                const guildMembers: Member[] = Object.values(membersData).map((m: any) => ({
                    id: m.id || m.name,
                    name: m.name,
                    class: m.class || '검성',
                    power: m.power || 0,
                ...
                onValue(appsRef, (appsSnap) => {
                    const appsData = appsSnap.val() || {};
                    const applications: any[] = Object.values(appsData);

                    // 3. Merge
                    const memberMap = new Map();
                    // Create a mutable copy of guild members
                    const mergedMembers = guildMembers.map(m => {
                        const clone = { ...m };
                        memberMap.set(clone.name, clone);
                        return clone;
                    });
                    ...
                    applications.forEach(app => {
                        const member = memberMap.get(app.nickname);
                        if (member) {
                            // Guild Member Applied: Update availability
                            member.availability = app.availability;
                        } else {
                            // Guest Applied
                            const guest: Member = {
                                id: app.id || `guest-${app.nickname}`,
                                name: app.nickname,
                                class: app.class || '검성',
                                power: app.power || 0,
                                rank: '게스트',
                    ...
                    const finalAllMembers = [...mergedMembers, ...guests];

                    setDbMembers(membersData);
                    setAllMembers(finalAllMembers); // Full roster with updated availability

                    // Filter valid applicants for the initial Pool
                    const initialPool = finalAllMembers.filter(m =>
                        m.availability && Object.values(m.availability).some(slots => slots && slots.length > 0)
                    );
    ...
    const handleAutoMatch = () => {
        ...
        const { matchType } = matchOptions;

        // Validation for Single Day Match
        if (matchType === 'DAY' && selectedDay === 'ALL') {
            alert("요일 선택 매칭을 위해서는 먼저 특정 요일을 선택해주세요.");
            return;
        }
        ...
        // 1. Determine Scope (Days to Process)
        const allDays = RAID_DAYS; // Mon-Sun (or user defined order)
        const targetDays = matchType === 'ALL' ? allDays : [selectedDay];

        // 2. Reset Logic (Release members to pool based on scope)
        if (matchType === 'ALL') {
            // Reset ALL: Clear everyone from parties
            ...
            // Reset Party List (Optional: Keep IDs or rebuild? Keep for now to preserve structure)
            // But we might need more parties. We'll handle expansion in loop.
        } else {
            // Reset DAY: Clear only parties assigned to this day (or unassigned if we treat them as 'free')
            // Actually, we should only clear parties that are explicitly assigned to targetDays?
            // Or if they are empty/unassigned, we can use them.
            workingParties.forEach(p => {
                if (targetDays.includes(p.assignedDay!)) {
                    if (p.members.length > 0) {
                        workingPool.push(...p.members);
                        p.members = [];
                    }
                    // Keep assignment? No, we might re-assign slots.
                    // p.assignedDay = undefined; // Let's keep vague for now, or reset.
                    // Resetting allows better redistribution.
                    p.assignedDay = undefined;
                    p.assignedTime = undefined;
                }
            });
        ...
        // Setup Pool map for faster lookup/dedup
        let poolMap = new Map(workingPool.map(m => [m.id, m]));

        // 3. Scheduler Loop (Scarcity-Based Optimization)
        // A. Collect all target slots & Calculate Scarcity
        type ScheduleSlot = { day: string; slotId: string; initialCount: number };
        let scheduleQueue: ScheduleSlot[] = [];
        ...
        targetDays.forEach(day => {
            ...
            slots.forEach(slot => {
                // Count availability for this slot
                const count = Array.from(poolMap.values()).filter(m =>
                    m.availability?.[day]?.includes(slot.id)
                ).length;

                if (count > 0) {
                    scheduleQueue.push({ day, slotId: slot.id, initialCount: count });
                }
            });
        });

        // B. Sort by Scarcity (Fewest Candidates First)
        scheduleQueue.sort((a, b) => a.initialCount - b.initialCount);

        // C. Execute Matching
        scheduleQueue.forEach(({ day, slotId }) => {
            // A. Identify Candidates (Re-fetch from Logic Pool)
            let candidates = Array.from(poolMap.values()).filter(m =>
                m.availability?.[day]?.includes(slotId)
            );
            ...
            // B. Determine Needed Parties
            // We need to find parties for this slot.
            // If 'ALL' mode, we fill sequentially. if 'DAY' mode, we might reuse cleared ones.
            // Strategy: Find parties already assigned to this (day, slot) - unlikely if we cleared.
            // Find Unassigned parties. 
            // Create new if needed.

            const neededCount = Math.ceil(candidates.length / 4);
            let assignedParties: Party[] = [];

            // 1. Find existing assigned (if any survived reset)
            assignedParties.push(...workingParties.filter(p => p.assignedDay === day && p.assignedTime === slotId));

            // 2. Assign Unassigned
            if (assignedParties.length < neededCount) {
                const unassigned = workingParties.filter(p => !p.assignedDay);
                const needed = neededCount - assignedParties.length;

                for (let i = 0; i < needed; i++) {
                    if (i < unassigned.length) {
                        const p = unassigned[i];
                        p.assignedDay = day;
                        p.assignedTime = slotId;
                        assignedParties.push(p);
                    } else {
                        // Create New
                        const newParty: Party = {
                            id: `party-${workingParties.length + 1}`,
                            name: `${workingParties.length + 1}파티`,
                            members: [],
                            assignedDay: day,
                            assignedTime: slotId
                        };
                        workingParties.push(newParty);
                        assignedParties.push(newParty);
                    }
                }
            }

            // If we have more candidates than slots * 4? 
            // We typically expand parties to fit candidates.
            // The above logic ensures we have enough *parties*.

            // C. Run Algo Pipeline
            // --- V1 Algorithm Implementations ---
            ...
            const matchMainTank = (partiesToMatch: Party[], candidates: Member[]) => {
                const used: string[] = [];
                const templars = candidates.filter(m => m.class === '수호성').sort((a, b) => b.power - a.power);
                const gladiators = candidates.filter(m => m.class === '검성').sort((a, b) => b.power - a.power);
                const tanks = [...templars, ...gladiators];

                // Check force-level tank presence? Here we match per slot (Force-ish).
                // We treat assignedParties as the matching group.

                // Simple logic: Ensure each party has 1 tank if possible.
                for (const p of partiesToMatch) {
                    if (p.members.some(m => ['수호성', '검성'].includes(m.class))) continue;
            ...
            // 4. Chanter Synergy
            const matchChanterSynergy = (partiesToMatch: Party[], candidates: Member[]) => {
                const used: string[] = [];
                let chanters = candidates.filter(m => m.class === '호법성').sort((a, b) => b.power - a.power);

                const assignChanter = (party: Party) => {
                    if (party.members.length >= 4) return false;
                    if (chanters.length === 0) return false;
                ...
                // Priority 1: Tank-less
                partiesToMatch.forEach(p => {
                    const hasTank = p.members.some(m => ['수호성', '검성'].includes(m.class));
                    const hasChanter = p.members.some(m => m.class === '호법성');
                    if (!hasTank && !hasChanter) assignChanter(p);
                });

                // Priority 2: General
                partiesToMatch.forEach(p => {
                    if (!p.members.some(m => m.class === '호법성')) assignChanter(p);
                });

                // Fill remaining
                if (chanters.length > 0) {
                    partiesToMatch.forEach(p => { if (chanters.length > 0) assignChanter(p); });
                }
                return used;
            };
            ...
            const matchCombatLogic = (partiesToMatch: Party[], candidates: Member[]) => {
                ...
                partiesToMatch.forEach(p => {
                    ...
                    let targetType = (melees > ranges + 1) ? 'RANGE' : ((ranges > melees + 1) ? 'MELEE' : '');
                    if (!targetType) return;

                    const pick = candidates.find(m =>
                        !used.includes(m.id) &&
                        (targetType === 'RANGE' ? ['마도성', '정령성', '궁성'].includes(m.class) : ['수호성', '검성', '살성'].includes(m.class))
                    );
                    if (pick) {
                        p.members.push(pick);
                        used.push(pick.id);
            ...
            const matchPowerBalance = (partiesToMatch: Party[], candidates: Member[]) => {
                ...
                while (sorted.length > 0) {
                    const targetParty = partiesToMatch
                        .filter(p => p.members.length < 4)
                        .sort((a, b) => {
            ...
            const runAlgo = (card: AlgoCard) => {
                let used: string[] = [];
                // Update candidates locally for this algo step
                const currentCandidates = candidates.filter(m => !used.includes(m.id)); // Actually filtering logic is below

                // We need to pass the *current* candidates (not used ones)
                // Logic mismatch: candidates variable in loop is local.
                // We need to filter it step by step.
            };

            // --- Fix: Pipeline Execution ---
            // We should update `candidates` after each step.

            const executePipeline = () => {
                const essentialCards = algoCards.filter(c => c.status === 'ESSENTIAL');
                const priorityCards = algoCards.filter(c => c.status === 'PRIORITY');
                const allCards = [...essentialCards, ...priorityCards];

                allCards.forEach(card => {
                    let used: string[] = [];
                    switch (card.id) {
                        case 'fixed_group': used = matchFixedParties(assignedParties, candidates, workingPool); break;
                        case 'main_tank': used = matchMainTank(assignedParties, candidates); break;
                        case 'resurrection_anchor': used = matchResurrectionAnchor(assignedParties, candidates); break;
                        case 'scarcity_priority': used = matchScarcityFill(assignedParties, candidates); break;
                        case 'safety_opt': used = matchChanterSynergy(assignedParties, candidates); break;
                        case 'combat_logic': used = matchCombatLogic(assignedParties, candidates); break;
                        case 'power_balance':
                        case 'attendance_volume': used = matchPowerBalance(assignedParties, candidates); break;
                    }

                    if (used.length > 0) {
                        // Remove used from candidates
                        candidates = candidates.filter(m => !used.includes(m.id));
                        // Also remove from poolMap so they aren't used in next slots?
                        used.forEach(uid => poolMap.delete(uid));
                    }
                });

                // Final Fallback if any left
                if (candidates.length > 0) {
                    const used = matchPowerBalance(assignedParties, candidates);
                    used.forEach(uid => poolMap.delete(uid));
                    candidates = [];
                }
            };
    ...
    setPool(Array.from(poolMap.values())); // Remaining pool
    alert(`매칭이 완료되었습니다. (${matchType === 'ALL' ? '전체 자동 매칭' : '요일 선택 매칭'})`);
};
...
const filteredPool = (() => {
    let baseMembers = pool;

    // If 'ALL' Days, include everyone (assigned + unassigned)
    if (selectedDay === 'ALL') {
        const assignedMembers = parties.flatMap(p => p.members);
        const allMap = new Map();
        // Add pool members
        pool.forEach(m => allMap.set(m.id, m));
        // Add assigned members (overwrite if exists, prioritizing assigned status visualization if needed, but Member object is same)
        assignedMembers.forEach(m => allMap.set(m.id, m));
        baseMembers = Array.from(allMap.values());
    }
                        ...
                        <section>
                            <div className="grid grid-cols-1 gap-3">
                                <label className={cn(
                                    ...
                                    matchOptions.matchType === 'DAY' ? "border-indigo-500 bg-indigo-50/50 dark:bg-indigo-900/10" : "border-slate-100 dark:border-slate-800"
                                )}>
                                    <input type="radio" name="scope" className="mt-1 accent-indigo-500 w-4 h-4"
                                        checked={matchOptions.matchType === 'DAY'}
                                        onChange={() => setMatchOptions(o => ({ ...o, matchType: 'DAY' }))} />
                                    <div>
                                        <span className="font-bold text-sm block mb-1">요일 선택 매칭</span>
                                        <p className="text-xs text-slate-500">현재 선택된 요일에 대해서만 매칭을 진행합니다.<br />다른 요일의 파티는 유지됩니다.</p>
                                    </div>
                                </label>
                                <label className={cn(
                                    "flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all hover:bg-slate-50 dark:hover:bg-slate-800",
                                    matchOptions.matchType === 'ALL' ? "border-rose-500 bg-rose-50/50 dark:bg-rose-900/10" : "border-slate-100 dark:border-slate-800"
                                )}>
                                    ...
                                    <div>
                                        <span className="font-bold text-sm block mb-1">전체 자동 매칭</span>
                                        <p className="text-xs text-slate-500">모든 요일(수~화)에 대해 순차적으로 매칭을 진행합니다.<br /><span className="text-rose-500 font-bold">※ 모든 파티 구성이 초기화됩니다.</span></p>
                                    </div>
                                </label>
                            </div>
                        </section>
