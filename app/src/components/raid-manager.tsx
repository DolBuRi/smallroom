        ...
        <div className="space-y-6 pb-20 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h2 className="text-4xl font-black text-slate-900 dark:text-white flex items-center gap-3 tracking-tight">
                        <Sword className="text-indigo-500 dark:text-indigo-400" size={32} />
                        성역 파티 도우미
                    </h2>
                    <div className="relative group inline-block">
                        <p className="text-slate-500 dark:text-slate-300 text-sm font-medium mt-[17px] flex items-center gap-2">
                ...
                <div className="flex gap-2">
                    {isAdmin && (
                        <button
                            ...
                            className="px-4 py-2 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-500 dark:text-indigo-300 rounded-xl text-xs font-black hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors flex items-center gap-2 border border-indigo-100 dark:border-indigo-500/30 shadow-sm"
                        >
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            isAdmin
                                ? "bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 border-red-100 dark:border-red-900/30"
                                : "bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 cursor-not-allowed border-slate-100 dark:border-slate-700 opacity-60"
                        )}
                    >
            ...
            {isGuideOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-[2.5rem] shadow-2xl w-full max-w-5xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[85vh] flex flex-col">
                        <div className="flex justify-between items-center p-8 pb-6 border-b border-slate-50 dark:border-slate-700 shrink-0">
                            <div>
                            ...
                            <button onClick={() => setIsGuideOpen(false)} className="bg-slate-50 dark:bg-slate-700 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-600 p-3 rounded-full transition-all">
                                <X size={24} />
                            </button>
                        </div>
            ...
            <div className="glass-panel p-2 flex flex-col md:flex-row items-center justify-between gap-4">
                {/* Day Tabs */}
                {/* [UI Fix] Removed overflow-x-auto to allow Today tag to float outside. Added flex-wrap for mobile safety. */}
                ...
                {/* View Toggles */}
                <div className="flex items-center gap-2 bg-slate-100/50 dark:bg-slate-900/50 p-1 rounded-xl">
                    <button
                        onClick={() => setViewMode('overview')}
                        className={cn(
                            "px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all",
                            viewMode === 'overview' ? "bg-white dark:bg-indigo-600 text-indigo-600 dark:text-white shadow-sm" : "text-slate-400 dark:text-slate-400 hover:text-slate-200"
                        )}
                    >
                    ...
                    <button
                        ...
                        className={cn(
                            "px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 transition-all",
                            viewMode === 'matching' ? "bg-white dark:bg-indigo-600 text-indigo-600 dark:text-white shadow-sm" : "text-slate-400 dark:text-slate-400 hover:text-slate-200"
                        )}
                    >
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <button
                                ...
                                className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-all active:scale-95 flex items-center gap-2"
                            >
                    ...
                    <div className="space-y-8">
                        {activeForces.length === 0 && (
                            <div className="text-center py-20">
                                <div className="w-24 h-24 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-6">
                                    <Layers className="text-slate-300 dark:text-slate-600" size={40} />
                                </div>
                                <h3 className="text-xl font-black text-slate-900 dark:text-slate-200 mb-2">매칭된 성역 포스가 없습니다</h3>
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                                {activeForces.map((force, idx) => (
                                    <div key={force.id} className={cn(
                                        ...
                                        {/* Force Header */}
                                        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-3 border-b border-slate-100 dark:border-slate-700/50 pb-4">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-3 text-slate-600 dark:text-slate-400 font-bold text-xs">
                                                    <div className="flex items-center gap-1.5">
                                                        <Calendar size={14} />
                                                        <span className="text-slate-900 dark:text-slate-200">
                                                            {force.day}요일 <span className="text-slate-500 dark:text-slate-500 font-normal">({getDayDate(force.day)})</span>
                                                        </span>
                                                    </div>
                                            ...
                                            <div className="flex items-center gap-4">
                                                <div className="text-right">
                                                    <span className="text-[11px] font-bold text-slate-400 dark:text-slate-500 block mb-0.5">총 평균</span>
                                                    <span className="text-xl font-black text-slate-900 dark:text-slate-100 tabular-nums leading-none">
                                                ...
                                                {isAdmin && (
                                                    <button onClick={() => handleDisband(force.id)} className="p-2 bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors">
                                                        <Trash2 size={16} />
                                                    </button>
                                                )}
                                            </div>
                                        ...
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                            {/* Party 1 */}
                                            <div className="bg-slate-50/50 dark:bg-slate-800/50 rounded-lg p-3 border border-slate-100/50 dark:border-slate-700/50">
                                                <div className="flex items-center justify-between text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 px-1">
                                                    <span className="text-indigo-600 dark:text-indigo-400">1파티</span>
                                            ...
                                            <div className="bg-slate-50/50 dark:bg-slate-800/50 rounded-lg p-3 border border-slate-100/50 dark:border-slate-700/50">
                                                <div className="flex items-center justify-between text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 px-1">
                                                    <span className="text-indigo-600 dark:text-indigo-400">2파티</span>
                        ...
                        {/* Unassigned / Waiting List */}
                        {
                            unassignedMembers.length > 0 && (
                                <div className="mt-8 border-t border-slate-200 dark:border-slate-700/50 pt-6 animate-in slide-in-from-bottom-5 duration-700 delay-200">
                                    {/* Header & Tabs */}
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                                        <div className="flex items-center gap-2 opacity-90">
                                            <h3 className="text-sm font-black text-slate-600 dark:text-slate-200 flex items-center gap-2">
                                                <AlertCircle size={16} /> 대기 인원
                                            </h3>
                                            <span className="bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded text-[11px] font-bold">
                                                {unassignedMembers.length}명
                                            </span>
                                        </div>
                                        <div className="flex bg-slate-100 dark:bg-slate-800/50 p-1 rounded-lg self-start md:self-auto">
                                            <button
                                                onClick={() => setWaitingViewMode('day')}
                                                className={cn(
                                                    "px-3 py-1.5 rounded-md text-xs font-bold transition-all",
                                                    waitingViewMode === 'day' ? "bg-white dark:bg-indigo-600 text-slate-900 dark:text-white shadow-sm" : "text-slate-400 dark:text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"
                                                )}
                                            >
                                                요일별 보기
                                            </button>
                                            <button
                                                ...
                                                className={cn(
                                                    "px-3 py-1.5 rounded-md text-xs font-bold transition-all",
                                                    waitingViewMode === 'list' ? "bg-white dark:bg-indigo-600 text-slate-900 dark:text-white shadow-sm" : "text-slate-400 dark:text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"
                                                )}
                                            >
                                                전체 명단
                                            </button>
                                                    ...
                                                    <div key={day} className="space-y-4">
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                                                            {currentSlots.map(slot => {
                                                                ...
                                                                return (
                                                                    <div key={slot.id} className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 rounded-lg p-3">
                                                                        <div className="flex justify-between items-center mb-2 pb-2 border-b border-slate-200/50 dark:border-slate-700/50">
                                                                            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">{slot.label}</span>
                                                                            <span className="bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 px-1.5 py-0.5 rounded text-[10px] font-black border border-indigo-100 dark:border-slate-600">
