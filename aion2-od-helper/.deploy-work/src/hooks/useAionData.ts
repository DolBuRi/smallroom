...
function isValidSyncKey(key: string | null | undefined) {
  return !!key && /^[A-Z0-9]{8}$/.test(key.trim().toUpperCase());
}

function withoutRuntimeId(item: any) {
  const { id, ...data } = item;
  return data;
}
...
export function useAionData() {
  ...
  const executeAction = async (id: string, type: string, isAccount: boolean = false) => {
    ...
    const lastUpdate = actionTime.toISOString();

    if (isAccount) {
      const account = accounts.find(a => a.id === id);
      if (!account) return;
      const calculatedAccount = calculateCurrentState(account, actionTime, false);
      const updates: any = { ...withoutRuntimeId(calculatedAccount), lastUpdate };
      if (actionType === 'shugo') updates.shugoBasic = Math.max(0, (calculatedAccount.shugoBasic || 0) + (isUndo ? 1 : -1));
      else if (actionType === 'invasion') updates.invasionBasic = Math.max(0, (calculatedAccount.invasionBasic || 0) + (isUndo ? 1 : -1));
      ...
      const calculatedChar = calculateCurrentState(char, actionTime, true, !!account.membership);
      const calculatedAccount = calculateCurrentState(account, actionTime, false);
      const odeCost = account.membership ? 80 : 40;
      const charUpdates: any = { ...withoutRuntimeId(calculatedChar), lastUpdate };
      ...
      await updateAccount(account.id, {
        ...withoutRuntimeId(calculatedAccount),
        [field]: Math.max(0, (calculatedAccount[field] || 0) + (isUndo ? -1 : 1)),
        lastUpdate
      });
    }
  };
  ...
  const manualAdjust = async (id: string, field: string, delta: number, isAccount: boolean = false) => {
    ...
    if (isAccount) {
      ...
      if (acc) {
        const calculatedAcc = calculateCurrentState(acc, actionTime, false);
        await updateAccount(id, {
          ...withoutRuntimeId(calculatedAcc),
          [field]: Math.max(0, (calculatedAcc[field] || 0) + delta),
          lastUpdate
        });
      }
    } else {
      ...
      if (char) {
        ...
        await updateCharacter(id, {
          ...withoutRuntimeId(calculatedChar),
          [field]: Math.max(0, (calculatedChar[field] || 0) + delta),
          lastUpdate
        });
      }
    }
  ...
  const toggleCheck = async (id: string, field: string, isAccount: boolean = false) => {
    const actionTime = new Date();
    const lastUpdate = actionTime.toISOString();
    const rawItem = isAccount 
      ? accounts.find(a => a.id === id) 
      : characters.find(c => c.id === id);
    const account = isAccount
      ? rawItem
      : accounts.find(a => a.id === rawItem?.accountId);
    const item = rawItem
      ? calculateCurrentState(rawItem, actionTime, !isAccount, !!account?.membership)
      : null;
    if (!item) return;
    ...
    if (isAccount) {
      ...
      if (maxMap[field]) {
        ...
        await updateAccount(id, { ...withoutRuntimeId(item), [field]: newValue, lastUpdate });
      } else {
      ...
      if (field === 'nightmare') {
        ...
        await updateCharacter(id, { ...withoutRuntimeId(item), [field]: newValue, lastUpdate });
        return;
      }
  ...
  const addCharacter = async (data: any) => {
    ...
    const newChar = {
      ...
      ode: account?.membership ? ODE_MAX_MEMBERSHIP : ODE_MAX_NORMAL,
      odeExtra: 0,
  ...
  const updateSyncKey = (key: string | null) => {
    ...
    if (!isValidSyncKey(newKey)) return;
