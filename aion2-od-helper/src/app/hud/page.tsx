...
export default function HudPage() {
  ...
  const [view, setView] = useState<"hud" | "settings" | "vision" | "alerter">("hud")
  ...
  const processAudioQueue = useCallback(() => {
    ...
    const { fileName, volume, msg } = item;
    remoteLog('info', `Playing sound: ${fileName} (Vol: ${volume})`);

    const audio = new Audio(`/assets/mp3/${fileName}`);
    audio.volume = volume;
    ...
    audio.onerror = (e) => {
      remoteLog('error', `Audio playback error for ${fileName}`, e);
      // TTS Fallback
      if (window.speechSynthesis) {
        const cleanMsg = msg.replace(/<[^>]*>/g, '').trim();
        const u = new SpeechSynthesisUtterance(cleanMsg);
        u.volume = volume;
        u.lang = 'ko-KR';
        u.onend = onComplete;
    ...
    audio.play().catch(err => {
      remoteLog('error', `Audio play failed for ${fileName}`, err);
      // TTS Fallback
      if (window.speechSynthesis) {
        const cleanMsg = msg.replace(/<[^>]*>/g, '').trim();
        const u = new SpeechSynthesisUtterance(cleanMsg);
  ...
  const playAionSound = useCallback((msg: string, fileName: string) => {
    if (!alerterConfig.enabled) return;
    const volume = (alerterConfig.volume ?? 80) / 100;
    audioQueueRef.current.push({ msg, fileName, volume, id: Date.now() });
    if (!isAudioPlayingRef.current) processAudioQueue();
  }, [alerterConfig.enabled, alerterConfig.volume, processAudioQueue]);
  ...
  }, [alerterConfig, syncKey, playAionSound]);
  ...
  const isValidKey = (key: string) => {
    ...
    if (cleanKey.length < 2) {
      remoteLog('warn', `isValidKey: Key too short (${cleanKey.length})`, cleanKey);
      return false;
    }
    return true;
  };
  ...
  useEffect(() => {
    ...
    const alerterPath = `users/${syncKey}/alerter`;
    ...
    let unsubAlerter: (() => void) | null = null;
    ...
    if (typeof window !== "undefined" && (window as any).api) {
      ...
      (window as any).api.subscribe(alerterPath);
      ...
      unregisterIPC = (window as any).api.onDataUpdate((payload: { path: string, data: any }) => {
        ...
        } else if (payload.path === alerterPath) {
          if (payload.data) setAlerterConfig((prev: any) => ({ ...prev, ...payload.data }));
        }
      ...
      const initWeb = async () => {
        ...
        try {
          ...
          unsubAlerter = onValue(ref(db, alerterPath), (snapshot) => {
            const data = snapshot.val();
            if (data) setAlerterConfig((prev: any) => ({ ...prev, ...data }));
          });
  ...
  const updateAlerterConfig = async (fields: Partial<AlerterConfig>) => {
    if (!syncKey) return;
    const updated = { ...alerterConfig, ...fields };
    setAlerterConfig(updated);
    await saveToFirebase(`users/${syncKey}/alerter`, updated);
  }
  ...
  return (
    ...
    <main className={cn("relative flex flex-col items-start p-4 w-[640px] h-[950px] bg-transparent text-white font-sans overflow-visible transition-opacity duration-300", !isHudVisible && "opacity-0 pointer-events-none")}>
      ...
      >
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-black text-indigo-400 bg-indigo-500/10 px-1.5 py-0.5 rounded tracking-tighter">v0.2.0</span>
      ...
      {/* --- ALERTER SETTINGS --- */}
      {view === "alerter" && (
        <div ref={sideRef} className="absolute top-0 left-[328px] w-[280px] bg-[#121826] border border-white/10 rounded-2xl shadow-2xl animate-in slide-in-from-left-4 duration-300 overflow-hidden flex flex-col h-[600px]">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-6">
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-xl">
                ...
                <button 
                  ...
                  className={cn(
                    ...
                    alerterConfig.enabled ? "bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]" : "bg-slate-700"
                  )}
                >
                  <div className={cn(
                    "absolute top-1 w-3 h-3 bg-white rounded-full transition-all duration-300",
                    alerterConfig.enabled ? "left-6" : "left-1"
                  )} />
                </button>
              </div>
              ...
              <div className="space-y-2 px-1">
                ...
                <input 
                  type="range" min="0" max="100" step="5"
                  value={alerterConfig.volume}
                  onChange={(e) => updateAlerterConfig({ volume: parseInt(e.target.value) })}
            ...
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-2">
                {[
                  ...
                  <button 
                    key={item.id}
                    onClick={() => updateAlerterConfig({ [item.id]: !alerterConfig[item.id as keyof AlerterConfig] })}
                    className={cn(
                      "flex items-center justify-between p-3 rounded-xl border transition-all duration-200 group",
                      alerterConfig[item.id as keyof AlerterConfig] 
                        ? "bg-white/5 border-white/10 hover:border-white/20" 
                        : "bg-black/20 border-white/5 grayscale opacity-50"
                    )}
                  >
                    <div className="flex items-center gap-3">
                      <div className={cn("p-1.5 rounded-lg bg-black/40", item.color)}>
                        <item.icon className="w-3.5 h-3.5" />
                      </div>
                      <span className="text-[11px] font-bold text-slate-300 group-hover:text-white">{item.label}</span>
                    </div>
                    {alerterConfig[item.id as keyof AlerterConfig] && <Check className="w-3.5 h-3.5 text-indigo-400" />}
                  </button>
                ))}
              </div>
            ...
            <section className="space-y-3">
              ...
              <button 
                onClick={() => {
                  playAionSound("알람 사운드 테스트입니다.", "web_test.mp3");
                }}
                className="w-full py-3 bg-indigo-500 hover:bg-indigo-600 rounded-xl text-[11px] font-bold text-white shadow-lg shadow-indigo-500/20 transition-all flex items-center justify-center gap-2 group"
              >
